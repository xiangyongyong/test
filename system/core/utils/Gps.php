...
class Gps {
    ...
    //解析GPS输出的原始数据为百度坐标
    public static function gps_bd09($location){
        $arr100 = array();
        $arr99 = explode(",", $location);
        $lat_du = substr($arr99[0],0,2);
        $lat_fen = substr($arr99[0], 2);
        $wgsLat = $lat_du+($lat_fen/60);
        $lon_du = substr($arr99[1],0,3);
        $lon_fen = substr($arr99[1], 3);
        $wgsLon = $lon_du+($lon_fen/60);
        return (new self)->wgs_bd09($wgsLat, $wgsLon);
    }

    //解析GPS输出的原始数据为高德坐标
    public static function gps_gcj02($location){
        $arr100 = array();
        $arr99 = explode(",", $location);
        $lat_du = substr($arr99[0],0,2);
        $lat_fen = substr($arr99[0], 2);
        ...
        $lon_fen = substr($arr99[1], 3);
        $wgsLon = $lon_du+($lon_fen/60);
        return (new self)->gcj_encrypt($wgsLat, $wgsLon);
    }

    //WGS to BD-09
    public function wgs_bd09($wgsLat, $wgsLon){
        $arr = $this->gcj_encrypt($wgsLat, $wgsLon);
        return $this->bd_encrypt($arr['lat'], $arr['lon']);
    }
    //WGS-84 to GCJ-02
